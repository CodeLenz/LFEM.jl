...
#
# Função para fazer a visualização da função aproximada φ(t)
#
function Aproximacao(c0::Vector,c1::Vector,Y::Vector,y::Float64)
   ...
   return (sum(c0[1:(contador-1)])+(sum(c1[1:(contador-1)])*y))
...
function Calcula_c_l(a0l::T,a1l::T,t0::T,y0::T) where T
    ...
    return  exp(cte)*(y0 + a0l/a1l -2/t0) # - 2*a1l*exp(-cte)*MyInt(cte,t0,tf)   expint(cte))

end

# Calcula a resposta y_t(t) em um intervalo
function Calcula_y_l(t_l::T,t::T,a0l::T,a1l::T,c_l::T) where T

    # Cte -a1l*t
    cte = -a1l*t

    -(a0l/a1l) + (2/t) + (2*a1l*exp(-cte)*MyInt(-a1l,t_l,t)) + c_l*exp(-cte)
...
function main()
  ...
  #################### AQUI ESTAMOS SÓ VERIFICANDO SE O φ(y) está OK ########################
  ...
  y(t) = Calcula_y_l(t_l,t,a0[1],a1[1],c_l)
